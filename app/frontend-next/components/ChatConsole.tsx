  ...
  async function send(text: string) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          text: "The assistant is not reachable right now. This project is a live, evolving build; the assistant backend may not be deployed in the environment you are viewing.",
        },
      ]);
    } finally {
      setLoading(false);
    ...
    >
      ...
      {error && (
        <p className="px-4 pb-3 text-xs text-[#f2a93b]">
          Tip: the assistant needs blog_assistant_stack.py deployed. See the project README.
        </p>
      )}
    </div>
  );
